...
extension String {
    ...
    func subString(from startIndex: Int, to endIndex: Int) -> String {
        ...
        let indexEndOfText = self.index(self.startIndex, offsetBy: endIndex)
...
struct Decompresser {
    ...
    func decompressedSequence() -> String {
        var decompressed = ""
        var sequenceIndex = 0
        while sequenceIndex < sequence.characters.count {
            if sequence.subString(at: sequenceIndex) != "(" {
                decompressed += sequence.subString(at: sequenceIndex)
                sequenceIndex += 1
                continue
            }
            
            var rangeIndex = sequenceIndex+1
            var rangeString = ""
            while sequence.subString(at: rangeIndex) != "x" {
                rangeString += sequence.subString(at: rangeIndex)
                rangeIndex += 1
            }
            
            var repatingIndex = rangeIndex + 1
            var repeatingString = ""
            while sequence.subString(at: repatingIndex) != ")" {
                repeatingString += sequence.subString(at: repatingIndex)
                repatingIndex += 1
            }
            
            let rangeValue = Int(rangeString)!
            let repeatingValue = Int(repeatingString)!
            
            let rangeCount = rangeString.characters.count
            let repeatingCount = repeatingString.characters.count
            
            let start = sequenceIndex + 3 + rangeCount + repeatingCount
            let end = start + rangeValue
            
            let repeatedString = sequence.subString(from: start, to: end)
            for _ in 0..<repeatingValue {
    ...
    // FIXME: Horrible solution - Don't even know if it works
    func decompressedSequenceV2() -> String {
        
        var decompressed2 = sequence
        var moreToCompress = true
        
        while moreToCompress == true {
            moreToCompress = false
            var decompressed = ""
            var sequenceIndex = 0
            while sequenceIndex < decompressed2.characters.count {
                if decompressed2.subString(at: sequenceIndex) != "(" {
                    decompressed += decompressed2.subString(at: sequenceIndex)
                    sequenceIndex += 1
                    print(sequenceIndex)
                    continue
                }
                ...
                var rangeIndex = sequenceIndex+1
                var rangeString = ""
                while decompressed2.subString(at: rangeIndex) != "x" {
                    rangeString += decompressed2.subString(at: rangeIndex)
                    rangeIndex += 1
                }
                
                var repatingIndex = rangeIndex + 1
                var repeatingString = ""
                while decompressed2.subString(at: repatingIndex) != ")" {
                    repeatingString += decompressed2.subString(at: repatingIndex)
                    repatingIndex += 1
                }
                ...
                let repeatedString = decompressed2.subString(from: start, to: end)
                for _ in 0..<repeatingValue {
                    decompressed += repeatedString
                }
                sequenceIndex = end
            ...
            decompressed2 = decompressed
        }
        
        return decompressed2
    }
    
    func decompressedLength() -> Int {
        return decompressedSequence().characters.count
    }
    
    func decompressedV2Length() -> Int {
        return decompressedSequenceV2().characters.count
    }
    ...
    func decompressedLength(of s: String, usingDecompressedMarkers m: Bool = false) -> Int {
        ...
        let a = (1...2).map { (value) -> Int in
            let range = matches[0].rangeAt(value)
            let string = (s as NSString).substring(with: range)
            return Int(string)!
        } // now a[0] is the sequence length, a[1] is the number of copies
        
        let start = matches[0].rangeAt(2).location + matches[0].rangeAt(2).length + 1
        let end = start + a[0]
        
        let stringAppliesTo2 = s.subString(from: start, to: end)
        let stringRemaining2 = s.subString(from: end)
        
        let stringBeforeMarker2 = s.subString(from: 0, to: matches[0].rangeAt(1).location - 1)
        let repeatedLength = m ? decompressedLength(of: stringAppliesTo2, usingDecompressedMarkers: true) : stringAppliesTo2.characters.count
        
        return stringBeforeMarker2.characters.count + (repeatedLength * a[1]) + decompressedLength(of: stringRemaining2, usingDecompressedMarkers: m)
    }
